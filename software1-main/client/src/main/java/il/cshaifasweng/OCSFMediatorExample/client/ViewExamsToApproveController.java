...
public class ViewExamsToApproveController extends PageController {
    ...
    void ShowStudentsWhoExecuteTheSelectedExam(ActionEvent event) {
        ...
        if (selected instanceof ExamExecutionDigital) {
            Platform.runLater(() -> {
        ...
        if (selected instanceof ExamExecutionManual) {
            Platform.runLater(() -> {
                setWindowTitle("List Of Students Who Execute An Exam");
                try {
                    this.getData().setLecturerExamExecutionLink_id(LecturerExamexecutionsLinkList.get(index).getLecturerExamexecutionsLink_id());
                    in_page = false;
                    setContent("TeacherGetSolvedWordFile", this.getData());
                } catch (IOException e) {
                    e.printStackTrace();
                }
            });
    ...
    public void setDataFromServerTF(MessageEvent event) throws IOException {
        if (event.getMessage().getController_name().equals("ViewExamsToApproveController")) {
            if (event.getMessage().getMessage().equals("exams_execution link to be approved given")) {
    ...
    public void refresh(UpdateMessageEvent event) {
        System.out.println("refresh ViewExamsToApproveController");
        System.out.println(event.getMessage().getMessage());

        if (event.getMessage().getController_name().equals("ViewExamsToApproveController") && in_page) {
            System.out.println(event.getMessage().getMessage());

            if (event.getMessage().getMessage().equals("exam execution have been ended ExamsToApprove check if you execute it")) {

                System.out.println("executor id: "+event.getMessage().getLecturerId() );
                System.out.println("your id: "+this.getData().getTeacher_id() );


                if (event.getMessage().getLecturerId() == this.getData().getTeacher_id()) {
                    //////
                    Message message = new Message("get exams_execution link by Teacher id waiting to be approved");
    ...
    void initialize1() {
        ...
        this.getData().setCurrentControllerName("ViewExamsToApproveController");
